...
Mirrors train.py as a two-step KFP v2 pipeline so each run is tracked
in the KFP UI with versioned inputs, outputs, and metrics.
...
@dsl.pipeline(
    ...
    description="Train and evaluate a RandomForest diabetes classifier.",
)
def diabetes_pipeline(
    data_url: str = "https://raw.githubusercontent.com/plotly/datasets/master/diabetes.csv",
):
    train_task = train_op(data_url=data_url)
    evaluate_op(
